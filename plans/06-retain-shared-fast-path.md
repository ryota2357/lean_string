...
`Repr::retain` (src/repr.rs:653-659) は本体に入る前に無条件で `ensure_modifiable` を呼ぶ。
...
「長さだけを縮めるなら共有のままでよい」を判定している、repr.rs:775-786) ので、
挙動としても非対称になっている。
...
- **述語が panic したときの状態**。走査フェーズではバッファを触っていないので、
  panic しても文字列は無傷。既存の `SetLenOnDrop` (repr.rs:661-708) は
...
- `Repr::remove` (repr.rs:614-651): 必ず 1 文字消すので現状のままで正しい。
- `Repr::insert_str` (repr.rs:715-749): 空文字列の早期 return が既にあり (repr.rs:722)、
  `8c1a40d` の `reserve(0)` no-op 化と合わせて問題ない。
